...
1.현재 위치에서 최단 거리 승객을 찾는다.(단, 연료가 부족할 시 실패(종료) )

2.승객들의 출발점에서 도착지점까지 연료를 다 소모하지 않고 이동하는지 확인한다.

조금 길었던 구현 문제

택시 클래스와 승객 클래스를 만들어 각각의 큐를 만들어 넣어주고 택시의 이동에 따라 움직이면서 연료 소모량을 체크한다.
